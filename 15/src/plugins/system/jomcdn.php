...
class plgSystemJomCDN extends JPlugin
{
	...
	function check_database_tables()
	{
		...
		if ( $db_version < 1002 ) {
			$query = "ALTER TABLE `#__jomcdn_files`
				ADD `file_type` VARCHAR( 127 ) NOT NULL DEFAULT 'default'
					COMMENT 'The type of file. images, css, js, or default' AFTER `id`,
				ADD INDEX ( `file_type` )";
			$this->_db->setQuery( $query );
			$this->_db->query();
			$new_version = 1002;
		...
		if ( $db_version < 1003 ) {
			$query = "ALTER TABLE `#__jomcdn_files`
				CHANGE `file_type` `cdn_loc` TINYINT( 3 ) UNSIGNED NOT NULL DEFAULT '0'
					COMMENT 'The cdn bucket where the file gets placed.'";
			$this->_db->setQuery( $query );
			$this->_db->query();
			$new_version = 1003;
		}
	...
	function replace_files()
	{
		...
		foreach ( $files as $file ) {
			...
			$replacements[] = $cdn_domains[$file->cdn_loc] . $rfile;
		}

		// Do actual URL replacements
		if ( !empty( $patterns ) && !empty( $replacements ) ) {
			$buffer = str_replace( $patterns, $replacements, $buffer );

			// Replace any whackiness1
			$buffer = str_replace( '/http', 'http', $buffer );

			// Replace any whackiness2
			if ( $cdn_domains[0] ) {
				$_uri = substr( JURI::root(), 0, -1 );
				$buffer = str_replace(
					array(
						$_uri . $cdn_domains[0],
						$_uri . $cdn_domains[1],
	...
	function get_stylesheets( $content )
	{
		...
		foreach ( $styles as $org_style_file => $abs_style_file ) {
			...
			// Start the use of only one cdn bucket
			$cdn->cdn_start_global_counter();

			// This will send all stylesheet type files to the CDN
			$this->parse_stylesheet( $styles, $org_style_file, $abs_style_file );

			// Stop the use of only one cdn bucket and get ready for the next one
			$cdn->cdn_stop_global_counter();
		}
	...
	function parse_stylesheet( &$styles, $org_style_file, $abs_style_file )
	{
		...
		// Font files
		foreach ( array( '.woff', '.ttf', '.svg', '.eot' ) as $_font_file ) {
			if ( strpos( $stylesheet_contents, $_font_file ) ) {
				if ( CDN_DEBUG ) {
					myPrint( 'Fonts in stylesheet, aborting.' );
				}
		...
		foreach ( $urls as $file => $file_path ) {
			...
			if ( '.css' == substr( $file_path, -4 ) ) {
				...
				// This means that no URLs where found, and we can push the file
				if ( 'empty' == $cdn_files ) {
	 ...
	 *
	 * For a correct connection, the Amazon access key, secret key and buckets must be set
	...
	function get_s3_object()
	{
		switch ( $this->params->get( 'cdn_type' ) ) {
			case 's3':
				...
				$s3_buckets    = array(
					trim( $this->params->get( 's3_bucket' ) ),
					trim( $this->params->get( 's3_bucket_1', $this->params->get( 's3_bucket' ) ) ),
					trim( $this->params->get( 's3_bucket_2', $this->params->get( 's3_bucket' ) ) ),
					trim( $this->params->get( 's3_bucket_3', $this->params->get( 's3_bucket' ) ) )
					);

				return new S3_CDN(
					$s3_access_key, $s3_secret_key, $s3_use_ssl, $s3_buckets,
			...
			case 'rs':
				...
				$rs_buckets  = array(
					trim( $this->params->get( 'rs_bucket' ) ),
					trim( $this->params->get( 'rs_bucket_1', $this->params->get( 'rs_bucket' ) ) ),
					trim( $this->params->get( 'rs_bucket_2', $this->params->get( 'rs_bucket' ) ) ),
					trim( $this->params->get( 'rs_bucket_3', $this->params->get( 'rs_bucket' ) ) )
					);

				return new RACKSPACE_CDN(
					$rs_api_key, $rs_username, $rs_buckets, get_object_vars( $this )
				);
			default:
				return;
				break;
	...
	function add_file( $original_file, $absolute_path, $cdn_file = '', $content_type = null )
	{
		...
		$query = "INSERT INTO #__jomcdn_files
			( `file`, `cdn_loc`, `file_path`, `remote_file`, `remote_file_gz`,
			`expiration`, `path_hash`, `page` )
				VALUES
				( " .$this->_db->Quote( $original_file ). ", '{$cdn_files['cdn_loc']}',
				" .$this->_db->Quote( $absolute_path ). ",
				" .$this->_db->Quote( $cdn_files['file'] ). ",
				" .$this->_db->Quote( $cdn_files['gz_file'] ). ", {$expiration},
...
class S3_CDN extends CDN_HELER
{
	 ...
	 * @param bool Use SSL
	 * @param string Amazon S3 buckets to use
	 * @return void
	 */
	function __construct( $s3_access_key, $s3_secret_key, $s3_use_ssl, $buckets, $config = array() )
	{
	 ...
	 */
	function put_object( $local_file, $cdn_file = '', $_content_type = null )
	{
		if ( '' == $cdn_file ) {
			$cdn_file = $local_file;
		}

		if ( !is_array( $_content_type ) ) {
			$headers = array();
		} else {
		...
		$cdn_loc = $this->get_cdn_location( $content_type );

		// Set bucket to use
		$bucket = $this->buckets[$cdn_loc];
	 ...
	 * @return string URL
	 */
	function get_domain( $cdn_loc = 0, $uri_check = true )
	{
		$is_ssl = false;
		...
		if ( trim( $this->params->get( 's3_cloudfront_domain' ) ) ) {
			...
			$url .= trim( $this->params->get( 's3_cloudfront_domain' . $domain ) );
		} else {
			$url .= $this->get_bucket( $cdn_loc ) . '.s3.amazonaws.com';
		}
...
class RACKSPACE_CDN extends CDN_HELER
{
	 ...
	 * @return void
	 */
	function __construct( $username, $api_key, $buckets, $config = array() )
	{
		foreach ( $config as $key => $value ) {
			$this->$key = $value;
		}
		...
		if ( defined( 'CDN_CRON_RUNNING' ) && CDN_CRON_RUNNING ) {
			...
			foreach ( $buckets as $bucket ) {
				$_rs = "rs_{$bucket}";
				$this->$_rs = $conn->create_container( $bucket );
				$this->$_rs->make_public();
			}
		}
	...
	function put_object( $local_file, $cdn_file = '', $_content_type = null )
	{
		...
		$asset = $this->$_rs->create_object( $cdn_file_path );
...
{
	/**
	 * This array keeps track of the next cdn bucket to be used
	 * They are categorized by type
	 * The global key, is for when we need to put multiple assets into one bucket
	 * as when we put in all of the files from a css file into a single bucket
	 * 
	 * @var array
	 */
	var $_cdn_counter = array(
		'default' => 0,
		'images'  => 0,
		'css'     => 0,
		'js'      => 0,
	...
	/**
	 * Tells get_cdn_location() to use the global counter instead of the specific
	 * asset type counter
	 * 
	 * @var bool
	 */
	var $_cdn_counter_use_global = false;
	...
	/**
	 * Gets the bucket name from the type of file
	 *
	 * @return string URL
	 */
	function get_bucket( $cdn_loc = 0 )
	{
		if ( isset( $this->buckets[$cdn_loc] ) ) {
			return $this->buckets[$cdn_loc];
		} else {
	...
	/**
	 * This is used to determine the file type
	 * The file type is used to determine to which bucket on the CDN we send the file to.
	 * This also gets stored in the db.
	 */
	function get_file_type( $content_type )
	{
		...
		if ( 'application/x-javascript' == $content_type ) {
			return 'js';
		}
	...
	/**
	 * Will get the next bucket for the given content type
	 */
	function get_cdn_location( $content_type, $override = null )
	{
		// If we are using the global counter just return
		if ( true == $this->_cdn_counter_use_global ) {
			return $this->_cdn_counter['global'];
		}
		...
		if ( $override ) {
			...
			// This is if we want to rotate through the cdn locations
			// by content type and not overall
			// $file_type = $this->get_file_type( $content_type );
			$file_type = 'default';
		}

		$return = $this->_cdn_counter[$file_type];
		$this->_cdn_counter[$file_type]++;
		if ( $this->_cdn_counter[$file_type] > 3 ) {
			$this->_cdn_counter[$file_type] = 0;
		}

		myPrint( "Getting CDN counter: {$return}." );

		return $return;
	}

	function cdn_start_global_counter()
	{
		myPrint( "Starting global counter at {$this->_cdn_counter['global']}" );

		$this->_cdn_counter_use_global = true;

		// Return the cdn to be used
		return $this->get_cdn_location( null );
	}

	function cdn_stop_global_counter()
	{
		myPrint( "Stopping global counter." );

		$this->_cdn_counter_use_global = false;

		// Return cdn that has been used and get the next one ready to go
		return $this->get_cdn_location( null, 'global' );
	}
